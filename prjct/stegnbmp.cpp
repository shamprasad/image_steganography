...
sprintf(command,"%s%s%s","bmptopnm ",argv[1]," >> tempimg");
system(command);
...
writePPM (argv[2], image);
       close(fd);
remove("tempimg");
sprintf(command,"%s%s%s","ppmtobmp -bpp=24 ",argv[2]," >> stegnimg/tempimg");
system(command);
remove(argv[2]);
rename("stegnimg/tempimg",argv[2]);
remove("stegnimg/tempimg");
printf("heello\n");
    }
